...
def build_cache(args):
    ...
    # Coverage generation must not require an already-built late-guard cache.
    ...
    for batch in loader:
        ...
        for idx, (dataset_name, stem) in enumerate(zip(batch["dataset"], batch["stem"])):
            ...
            print(
                f"[LCEG Cover] {dataset_name}/{stem} | area={cover_area:.6f} | "
                f"path={out_path}",
                flush=True,
            )
    ...
    print(
        f"[LCEG Cover] done | rows={len(rows)} | written={num_written} | manifest={manifest_path}",
        flush=True,
    )
