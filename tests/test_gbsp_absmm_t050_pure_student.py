...
from common.r1hard_linear_pure_student import (
    ...
    is_gbsp_absmm_t050_pure_student_config,
)
from common.utils import load_config


class GBSPAbsMinMaxT050PureStudentTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.cfg = load_config(GBSP_ABSMM_T050_CONFIG_PATH)
    ...
    def test_config_contract(self):
        ...
        self.assertTrue(is_gbsp_absmm_t050_pure_student_config(self.cfg))
        self.assertEqual(report["contract"]["source"], "gbsp_abs_minmax_37")
        self.assertEqual(report["contract"]["target"], "strict_greater_than_0.5")
        self.assertEqual(report["contract"]["student"], "single_1x1_conv")
        self.assertFalse(self.cfg.GBSP_ORACLE_THRESHOLD_VERIFICATION)
        self.assertTrue(self.cfg.GBSP_THRESHOLD_GT_FREE)
